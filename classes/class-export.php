...
if ( isset($_GET['myAction'] ) ) 
{
	...
	switch ($myAction)
	{
		case "exportEKstatsUserList":
			// Handle CSV Export
			add_action( 'admin_init', array('ekStats_export', 'exportEKstatsUserList') );
...
class ekStats_export
{
	
	
	
	static function exportEKstatsUserList()
	{
		...
		$CSV_array = ek_user_stats_draw::drawAllStudentsLog(true); // True means downalod as CSV
	...
	static function generateExportFile()
	{
		...
		// Check permissions	
		if(!current_user_can(get_option('min_quiz_access_level', 'delete_others_pages')))
		{
			return;
		}
	...
	public static function csv_export()
	{
		...
		if(!current_user_can(get_option('min_quiz_access_level', 'delete_others_pages'))){ return false; }
